function [allHFunctions,allHValues,allBckts] = LSHimplementation203(Path,noLayers,noHashs)
    files = dir(Path);
    isDir = [files(:).isdir];
    ...
    tempFile = csvread(strcat(Path,'\Outputs\Phase2-Task1\',folders{1},'-Projected\',csvFiles(1).name));
    nDims = size(tempFile,2);
        ...
        for i=1:num_files
            ...
            for j=1:noHashs
                ...
                for folder=1:folderCount
                   tfIdfFile = csvread(strcat(Path,'\Outputs\Phase2-Task1\',folders{folder},'-Projected\',csvFiles(i).name));
                   product = unitR(j,:) * transpose(tfIdfFile);
