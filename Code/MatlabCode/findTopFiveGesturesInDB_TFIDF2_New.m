function topFive = findTopFiveGesturesInDB_TFIDF2_New(Path,FileName)
   files = dir(Path);
   isDir = [files(:).isdir];
   ...
   for folder=1:foldersCount
       testFileTF = csvread(strcat(Path,'\test\',folders{folder},'\',FileName,'_TF-IDF2.csv'));
       nColumns = size(testFileTF,2);
       inputPath = strcat(Path,'\Outputs\Phase2-Task1\',folders{folder},'_TF-IDF2\','*.csv');
       files = dir(inputPath);
       ...
       for i=1:num_files
            ...
            normProducts = [];
            cosSimilarity = [];
            sheetTF = csvread(strcat(Path,'\Outputs\Phase2-Task1\',folders{folder},'_TF-IDF2\',files(i).name));
...
%    sortedIndex
   ...
   fid=fopen(strcat(Path,'\Outputs\Phase2-Task1\1C\',FileName,'_DB_topFiveFiles-TFIDF2.csv'),'wt');
